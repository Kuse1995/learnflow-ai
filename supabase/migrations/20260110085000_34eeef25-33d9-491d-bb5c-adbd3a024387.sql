-- Fix hard_delete_school_cascade to handle all related tables properly
...
BEGIN
  ...
  DELETE FROM student_learning_timeline WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id);
  ...
  -- Delete parent insight summaries
  ...
  -- Delete lesson data (lesson_resources has lesson_id, not class_id)
  DELETE FROM lesson_differentiation_suggestions WHERE class_id IN (SELECT id FROM classes WHERE school_id = p_school_id);
  -- lesson_resources doesn't have class_id - skip or handle differently if needed
  ...
  -- Delete teacher onboarding and feedback data
  DELETE FROM teacher_onboarding WHERE school_id = p_school_id;
  DELETE FROM teacher_feedback WHERE school_id = p_school_id;
  DELETE FROM teacher_quick_feedback WHERE school_id = p_school_id;
  DELETE FROM teacher_training_progress WHERE teacher_id IN (SELECT user_id FROM user_roles WHERE school_id = p_school_id AND role = 'teacher');
  DELETE FROM teacher_feature_first_use WHERE teacher_id IN (SELECT user_id FROM user_roles WHERE school_id = p_school_id AND role = 'teacher');
  ...
  DELETE FROM student_fee_balances WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id);
  ...
  -- Delete payment plan data
  DELETE FROM payment_installment_allocations WHERE installment_id IN (
    SELECT id FROM payment_plan_installments WHERE plan_id IN (
      SELECT id FROM payment_plans WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id)
    )
  );
  DELETE FROM payment_plan_workflow_history WHERE plan_id IN (
    SELECT id FROM payment_plans WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id)
  );
  DELETE FROM payment_plan_installments WHERE plan_id IN (
    SELECT id FROM payment_plans WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id)
  );
  DELETE FROM payment_plans WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id);
  DELETE FROM payment_plan_templates WHERE school_id = p_school_id;
  
  -- Delete guardian and parent related data
  DELETE FROM guardian_phone_registry WHERE guardian_id IN (SELECT id FROM guardians WHERE school_id = p_school_id);
  DELETE FROM parent_permissions WHERE guardian_id IN (SELECT id FROM guardians WHERE school_id = p_school_id);
  DELETE FROM parent_preference_history WHERE guardian_id IN (SELECT id FROM guardians WHERE school_id = p_school_id);
  DELETE FROM parent_contacts WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id);
  DELETE FROM student_guardians WHERE student_id IN (SELECT id FROM students WHERE school_id = p_school_id);
  ...
  -- Delete message and communication data
  DELETE FROM message_edit_history WHERE message_id IN (SELECT id FROM parent_messages WHERE school_id = p_school_id);
  DELETE FROM message_approval_log WHERE message_id IN (SELECT id FROM parent_messages WHERE school_id = p_school_id);
  ...
  DELETE FROM message_queue WHERE school_id = p_school_id;
  DELETE FROM message_templates WHERE school_id = p_school_id;
  DELETE FROM offline_message_queue WHERE school_id = p_school_id;
  ...
  DELETE FROM offline_sync_queue WHERE school_id = p_school_id;
  DELETE FROM offline_export_queue WHERE school_id = p_school_id;
  ...
  DELETE FROM school_offboarding_requests WHERE school_id = p_school_id;
  DELETE FROM school_system_history WHERE school_id = p_school_id;
  
  -- Delete restore requests and jobs
  DELETE FROM restore_jobs WHERE school_id = p_school_id;
  DELETE FROM restore_requests WHERE school_id = p_school_id;
  
  -- Delete demo users for this school
  DELETE FROM demo_users WHERE school_id = p_school_id;
